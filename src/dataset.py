...
# --------------- columnas del CSV que **sí** son entrada ------------------
CSV_FEATURES = [
...
class HormoneDataset(Dataset):
    """
    Devuelve dict con:
        x      : (S,F) features normalizadas
        y      : (S,)  e2 en pg/mL
        mask   : (S,)  bool
        days   : (S,)  entero 1-28 (o 1-33 si forecast)
        phase  : (S,)  0=foll,1=ovu,2=lutea
        pid    : ()    índice paciente
    """
    def __init__(self, df: pd.DataFrame):
        ...
        df["phase_idx"] = np.select(
            [df["measure_day"] <= 9,
             df["measure_day"] <= 14],
            [0, 1],               # 0=folicular,1=ovulatoria
            default=2)            # 2=lútea

        self.inp_cols = CSV_FEATURES + ["pos_sin", "pos_cos"]
        feats = df[self.inp_cols].astype(float)
        self.means, self.stds = feats.mean(), feats.std().replace(0, 1.0)
    ...
    def __getitem__(self, idx):
        g = self.groups[idx]
        L,S,F = len(g), self.max_len, len(self.inp_cols)
        ...
        x[:L]   = g[self.inp_cols].to_numpy(np.float32)
        y[:L]   = g["e2"].to_numpy(np.float32)
        day[:L] = g["measure_day"].to_numpy(np.int64)
        ph[:L]  = g["phase_idx"].to_numpy(np.int64)
        msk[:L] = True
